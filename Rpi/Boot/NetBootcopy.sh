...
export ROOT_FS=${NFS_ROOT}/root
export BOOT_FS=${ROOT_FS}/boot
...
#dwc_otg.lpm_enable=0 console=serial0,115200 console=tty1 root=/dev/nfs nfsroot=${HOST_IP}:/root,vers=${NFS_VERSION},proto=tcp,port=2049 rw ip=dhcp elevator=deadline rootwait plymouth.ignore-serial-consoles noswap #init=/bin/ro-root.sh
#pi0 USB (NFSv.3)
dwc_otg.lpm_enable=0 console=serial0,115200 console=tty1 root=/dev/nfs nfsroot=10.42.0.1:/nfs/root rw ip=10.42.0.14:10.42.0.1::255.255.255.0:pi:usb0:static elevator=deadline modules-load=dwc2,g_ether fsck.repair=yes rootwait g_ether.host_addr=5e:a1:4f:5d:cf:d2
EOF
}
...
prepare_bootconfig() {
    ...
    ${SUDO} bash -c "cat >> ${BOOT_FS}/config.txt" << EOF

[pi0]
cmdline=cmdline.pi0                     # PxeServer
# enable OTG                            # PxeServer
dtoverlay=dwc2                          # PxeServer
# set initramfs                         # PxeServer
# initramfs initrd.img followkernel     # PxeServer
# [0x12345678]
# cat /proc/cpuinfo 0000000012345678
...
EOF
    ${SUDO} mkdir -p ${ROOT_FS}/mnt/LinuxData/OF
    ${SUDO} chown 1000:1000 ${ROOT_FS}/mnt/LinuxData/OF
}

configure_nfs() {
    ${SUDO} sed -i '/PxeServer/d' /etc/exports
    if [ $NFS_VERSION -lt 4 ];then
    ${SUDO} bash -c "cat >> /etc/exports" << EOF
${ROOT_FS} *(rw,sync,no_subtree_check,no_auth_nlm,insecure,no_root_squash)             #PxeServer
EOF
    else 
    ${SUDO} bash -c "cat >> /etc/exports" << EOF
...
create_ssh_keypair() {
    # SUDO=sudo
    # ROOT_FS=/nfs/root
    mkdir -p -m=700 ${ROOT_FS}/home/pi/.ssh
...
# smsc95xx.macaddr=B8:27:EB:2D:91:D5 pi0
...
pepe() {
    ...
    # delete all trailing blank lines at end of file
    sed -e :a -e '/^\n*$/{$d;N;ba' -e '}' ${BOOT_FS}/config.txt
    echo "cmdline=netboot.txt" | ${SUDO} tee -a ${BOOT_FS}/config.txt
}
