...
[ -f /etc/wpa_supplicant/wpa_supplicant.conf.orig ] || \
...
echo "by adding \"logo.nologo vt.global_cursor_default=0\" at the end of the line in \"/boot/cmdline.txt\".\n"
    ...
    RASPBIAN_TYPE && echo "Raspbian Lite detected!!\n" && \
...
install_app () {
    cd
    git clone https://github.com/PiMakers/Works.git
    ...
    rm -r TothIlonkaApp/videos/SplittedVideos
    ##TODO Add startup script to .bashrc and silent boot 
    hopp(){
        case $(tty) in
            /dev/tty1)
            cd /home/pi/alkalmazas
            node main.js
        esac
        #startx hostname  pwd overlock boot memorysplit ssh ...
    }
}
...
remove_unused() {
# err=$(${SUDO} apt -y purge libreoffice* minecraft-pi wolfram-engine scratch 2>&1) 		|| echo "ERROR: $err"
}
